...
/// @dev Extension of {ERC20} that allows to implement user account transfer restrictions through
///   the {canTransact} function. Freely adapted from
///   `openzeppelin/community-contracts/token/ERC20/extensions/ERC20Restricted.sol`.
/// @dev By default, each account has no explicit restriction. The {canTransact} function acts as a
///   blocklist. Developers can override {canTransact} to check that `restriction == ALLOWED` to
...
abstract contract ERC20Restricted is ERC20 {
  ...
  /// @notice Returns whether a user account is allowed to interact with the token.
  /// @dev Default implementation only disallows explicitly BLOCKED accounts (i.e. a blocklist).
  /// @dev To convert into an allowlist, override as:
  ///
  ///    ```solidity
  ...
  /// @notice Updates the restriction of a user account.
  ...
  /// @notice Convenience function to block a user account (set to BLOCKED).
  function _blockUser(address account) internal virtual {
    _setRestriction(account, Restriction.BLOCKED);
  }

  /// @notice Convenience function to allow a user account (set to ALLOWED).
  function _allowUser(address account) internal virtual {
    _setRestriction(account, Restriction.ALLOWED);
  }

  /// @notice Convenience function to reset a user account to default restriction.
  function _resetUser(address account) internal virtual {
    _setRestriction(account, Restriction.DEFAULT);
  }

  /// @notice Checks if a user account is restricted. Reverts with {ERC20Restricted} if so.
